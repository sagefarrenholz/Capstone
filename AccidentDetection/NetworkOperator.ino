...
void setup_network_operator()
{
  ...
  const void* mem_test = calloc(120, 1);
  if (mem_test == NULL) {
    Serial.println(F("frick"));
  }
  /*
  mobile_network_operator_t network_operator;
  if (!mySARA.setNetworkProfile(MNO_SIM_ICCID))
  {
    Serial.println(F("Error setting network to ATT. Try cycling the power."));
  } else {
    network_operator = MNO_SIM_ICCID;
    goto found_operator;
  }*/

  /*if (!mySARA.setNetworkProfile(MNO_VERIZON))
  {
    Serial.println(F("Error setting network to VERIZON. Try cycling the power."));
  } else {
    network_operator = MNO_VERIZON;
     goto found_operator;
  }

  if (!mySARA.setNetworkProfile(MNO_TMO))
  {
    Serial.println(F("Error setting network to T mobile. Try cycling the power."));
  } else {
    network_operator = MNO_VERIZON;
    goto found_operator;
  }

  if (!mySARA.setNetworkProfile(MNO_SPRINT))
  {
    Serial.println(F("Error setting network to Sprint. Try cycling the power."));
    while (1) ;
  } else {
    network_operator = MNO_SPRINT;
    goto found_operator;
  }*/
